...
export interface TimingProviderProps {
  children: ReactNode;
...
const TimingContext = createContext<TimingClock | null>(null);
...
export function useTimingClock(): TimingClock {
  const context = useContext(TimingContext);
  if (!context) {
    throw new Error("useTimingClock must be used within a TimingProvider");
  }
  return context;
}

// Hook for components that might not be wrapped in TimingProvider
export function useOptionalTimingClock(): TimingClock | null {
